...
/* ---------------- MMIO helpers ---------------- */
static inline uint32_t _pmod_read(void)        { return IO_IN(IO_PMOD); }
static inline void     _pmod_write(uint32_t v) { IO_OUT(IO_PMOD, v); }
...
static inline void pmod_set(uint32_t m) { _pmod_write(_pmod_read() |  m); }
static inline void pmod_clr(uint32_t m) { _pmod_write(_pmod_read() & ~m); }
...
static inline void ssd1331_sclk_lo(void)        { pmod_clr(SSD1331_SCLK);  }
static inline void ssd1331_sclk_hi(void)        { pmod_set(SSD1331_SCLK);  }
static inline void ssd1331_mosi_lo(void)        { pmod_clr(SSD1331_MOSI);  }
static inline void ssd1331_mosi_hi(void)        { pmod_set(SSD1331_MOSI);  }
...
_fast static void ssd1331_spi_send(uint8_t byte) {
  for (int i = 0; i < 8; i++) {
    if (byte & 0x80) ssd1331_mosi_hi();
    else ssd1331_mosi_lo();
...
_fast static void ssd1331_cmd2(uint8_t cmd, uint8_t a, uint8_t b) {
  ...
  ssd1331_cs_assert();
  ssd1331_spi_send(cmd);
  ssd1331_spi_send(a); ssd1331_spi_send(b);
  ssd1331_cs_deassert();
}
...
_fast static void ssd1331_cmd3(uint8_t cmd, uint8_t a, uint8_t b, uint8_t c) {
  ...
  ssd1331_cs_assert();
  ssd1331_spi_send(cmd);
  ssd1331_spi_send(a); ssd1331_spi_send(b); ssd1331_spi_send(c);
  ssd1331_cs_deassert();
}
...
_fast static void ssd1331_cmd4(uint8_t cmd, uint8_t a, uint8_t b, uint8_t c, uint8_t d) {
  ...
  ssd1331_cs_assert();
  ssd1331_spi_send(cmd);
  ssd1331_spi_send(a); ssd1331_spi_send(b); ssd1331_spi_send(c); ssd1331_spi_send(d);
  ssd1331_cs_deassert();
}

/* ---------------- Pixel streaming helpers (DC=1 only for pixel bytes) ---------------- */
static inline void ssd1331_stream_begin(void)       { ssd1331_dc_data(); ssd1331_cs_assert(); }
